...
class TestVideoService:
    ...
    def test_adicionar_creditos(self, app, sample_video):
        ...
        with app.app_context():
            ...
            success, message = VideoService.adicionar_creditos(
                sample_video.id, quantidade
            )
            assert success == True
    ...
    def test_registrar_visualizacao(self, app, sample_video):
        ...
        with app.app_context():
            creditos_iniciais = sample_video.creditos
            visualizacoes_iniciais = sample_video.visualizacoes
            ...
            assert video.visualizacoes == visualizacoes_iniciais + 1

    def test_registrar_visualizacao_video_pausado(self, app, sample_video):
        """Testa registro de impressão em vídeo pausado"""
        with app.app_context():
    ...
    def test_registrar_visualizacao_sem_creditos(self, app, sample_video):
        """Testa registro de impressão sem créditos"""
        with app.app_context():
    ...
    def test_get_videos_by_location(self, app):
        """Testa busca de mídias (vídeos e imagens) por localização"""
        with app.app_context():
            # Criar vídeo em São Paulo
            video = Video(
                filename="sp.mp4",
                original_filename="sp.mp4",
                latitude=-23.5505,
                longitude=-46.6333,
                radius_km=50,
                aprovado=True,
            ...
            # Criar imagem em São Paulo
            image = Video(
                filename="sp.jpg",
                original_filename="sp.jpg",
                latitude=-23.5505,
                longitude=-46.6333,
                radius_km=50,
                aprovado=True,
            ...
            assert len(videos) >= 2  # Deve retornar vídeo e imagem
...
class TestClienteService:
    ...
    def test_autenticar_cliente_senha_errada(self, app, sample_cliente):
        ...
        with app.app_context():
            cliente = ClienteService.autenticar_cliente(
                sample_cliente.email, "senha_errada"
            )
    ...
    def test_autenticar_cliente_email_inexistente(self, app):
        ...
        with app.app_context():
            cliente = ClienteService.autenticar_cliente(
                "inexistente@example.com", "senha123"
            )
