...
def dashboard():
    ...
    return render_template(
        "admin.html", form=form, videos=videos, last_update=last_update
    )
        ...
        with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zipf:
            for root, dirs, files in os.walk(client_folder):
                ...
                dirs[:] = [
                    d
                    for d in dirs
                    if d not in ["venv", "__pycache__", ".pytest_cache", "downloads"]
                ]
                ...
                for file in files:
                    ...
                    arcname = os.path.join(
                        "client", os.path.relpath(file_path, client_folder)
                    )
...
@admin_bp.route("/stats/<int:video_id>")
@admin_required
