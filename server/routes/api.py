...
def download_video(video_id):
    """
    Baixa uma mídia específica (vídeo ou imagem)
    ...
    mimetype, _ = mimetypes.guess_type(video.filename)

    return send_from_directory(
        current_app.config["UPLOAD_FOLDER"],
        video.filename,
        as_attachment=False,  # False para permitir visualização inline no navegador
...
def registrar_visualizacao(video_id):
    ...
    if success:
        ...
        status_code = (
            403
            if "pausado" in message.lower() or "aprovado" in message.lower()
            else 402
        )
        return (
            jsonify(
