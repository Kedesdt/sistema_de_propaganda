...
def login():
    """Login do cliente"""
    # Se já está logado e o cliente existe, redirecionar para dashboard
...
def register():
    ...
    if request.method == "POST":
        ...
        current_app.logger.info(
            f"Form data após parse: nome={form.nome.data}, email={form.email.data}, endereco={form.endereco.data}"
        )
...
def upload_video():
    ...
    if form.validate_on_submit():
        ...
        if video:
            flash(
                f'Vídeo "{video.filename}" enviado com sucesso! Aguardando aprovação do admin.',
                "success",
            )
        else:
            flash(f"Erro: {error}", "danger")
